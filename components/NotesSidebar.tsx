...
export const NotesSidebar = ({
	...
	const [openTopics, setOpenTopics] = useState<Record<string, boolean>>(() =>
		Object.fromEntries(
			navigation.flatMap((subjectItem) =>
				subjectItem.topics.map((topicItem) => [
					`${subjectItem.subject}:${topicItem.topic}`,
					activeSubject === subjectItem.subject &&
	...
	useEffect(() => {
		setOpenSubjects((current) => {
			...
			for (const subjectItem of navigation) {
				next[subjectItem.subject] =
					activeSubject === subjectItem.subject ||
					current[subjectItem.subject] === true;
			}
		...
		setOpenTopics((current) => {
			...
			for (const subjectItem of navigation) {
				for (const topicItem of subjectItem.topics) {
	...
	}, [activeSubject, activeTopic, navigation]);
